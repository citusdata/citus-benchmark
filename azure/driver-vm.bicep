...
var sedCommands = format(sedCommandsTemplate, buildWarehouses, runWarehouses, buildVirtualUsers, runVirtualUsers, allWarehouses, duration, rampup, timeprofile)
...
git clone https://github.com/citusdata/ch-benchmark.git --branch improvements-jelte
cd ch-benchmark

{5}

while ! psql -c 'select 1'; do  echo failed; sleep 1; done
tmux new-session -d -t main -s cloud-init \; send-keys './build-and-run.sh {5}' Enter
'''
